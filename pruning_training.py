...
def main():
    ...
    torch.onnx.export(model, dummy_input, "VDSR_prepruning.onnx", verbose=True, input_names=input_names, output_names=output_names)        
    
    def trainer(model, optimizer, criterion, epoch):
        return train(training_data_loader, optimizer, model, criterion, epoch= epoch)
    ...
    if opt.pruner == 'level':
        ...
        # {
        #     'exclude': True,
        #     'op_names': ['input','output']
        # }
        ]

    elif opt.pruner not in ('l1filter', 'l2filter', 'fpgm'):
    ...
    model_path = 'pruned_{}_{}.pth'.format(opt.pruner,opt.sparsity)
    mask_path = 'mask_{}_{}.pth'.format(opt.pruner,opt.sparsity)
    ...
    input_names = [ "actual_input_1" ] + [ "learned_%d" % i for i in range(13) ]
    output_names = [ "output1" ]
    torch.onnx.export(model, dummy_input, "VDSR_postpruning_{}_{}.onnx".format(opt.pruner,opt.sparsity), verbose=True, input_names=input_names, output_names=output_names)
